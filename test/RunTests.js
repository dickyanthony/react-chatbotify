...
const sendName = async () => {
  ...
  await textAreaElement.sendKeys('Dicky Anthony', Key.RETURN);
  const expectedText = 'Hey Dicky Anthony!';
  await sleep(WAIT_DURATION);
...
const toggleNotifications = async () => {
  ...
  await sleep(WAIT_DURATION);
  const notificationOffElement = await driver.findElement(
    By.className('rcb-notification-icon-off')
  );
...
const viewHistory = async () => {
  ...
  if (replyElements.length != expectedReplyCount) {
    throw new Error(
      `Load history failed: Expected ${expectedReplyCount} ` +
  ...
  if (sendElements.length != expectedSendCount) {
    throw new Error(
      `Load history failed: Expected ${expectedSendCount} ` +
...
const charLimit = async () => {
  const charCounterElements = await driver.findElements(
    By.className('rcb-chat-input-char-counter')
  );
  if (charCounterElements.length > 0) {
    ...
    if (textAreaValue.length > characterLimit) {
      throw new Error(
        `Character limit exceeded: Expected max ${characterLimit} characters but found ${textAreaValue.length}`
      );
    }
  }
};
...
const scrollToBottom = async () => {
  ...
  if (!isScrolledToBottom) {
    throw new Error(
      'Chat did not auto-scroll to the bottom when the new message prompt was clicked.'
    );
  }
};
